...
JsonpMainTemplatePlugin.prototype.apply = function(mainTemplate) {
	...
	mainTemplate.plugin("jsonp-script", function(_, chunk, hash) {
		...
		var retriesCount = Number(this.outputOptions.chunkRetriesCount) || 0;
		return this.asString([
			"var script;",
			"function loadScript(retriesCount) {",
			this.indent([
			...
			"};",
			"loadScript(" + (retriesCount > 0 ? retriesCount : 0) + ")"
		]);
	});
